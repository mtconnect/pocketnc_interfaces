...
#from pnc_bridge import pocketNCAdapter
    ...
    class StateMachineModel:
        ...
        def initiate_cnc_client(self):
            if self.sim == False:
                ...
                self.cnc_client = pocketNCAdapter(str(device['host']),int(device['port']))
            elif self.sim == 'Mock':

                self.cnc_client = Mock()
                self.cnc_client._exec.value = MagicMock(side_effect = self.side_effect)
